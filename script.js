(function(venn) {
    ...
    venn.drawD3Diagram = function(element, dataset, width, height, parameters) {
        ...
        var labels = svg.selectAll(".label")
            ...
            .attr("y", function(d) { return d.y - (d.radius + 10); }) // Offset above the circle
            .text(function(d) { return d.label; })
            .style("fill", function(d, i) { return circleFillColours(i); }); // Set color based on index
        ...
        svg.selectAll(".line")
            ...
            .attr("y2", function(d) { return d.y - (d.radius + 10); })
            .attr("stroke", "black");

        // Implement force simulation for labels
...
function main() {
    var sets = [
        {"label": "Monotheistic", "size": 1000},
        {"label": "Polytheistic", "size": 1000},
        {"label": "Pantheistic", "size": 1000},
        {"label": "Animistic", "size": 1000},
        {"label": "Agnostic/Atheistic", "size": 1000},
        // Disjoint sets should not be included in overlaps
    ];
    
    var overlaps = [
        {"sets": [0, 1], "size": 200}, // Overlap between Monotheistic and Polytheistic
        {"sets": [1, 2], "size": 300}, // Overlap between Polytheistic and Pantheistic
        {"sets": [2, 3], "size": 150}, // Overlap between Pantheistic and Animistic
        {"sets": [0, 4], "size": 100}, // Overlap between Monotheistic and Agnostic/Atheistic
        {"sets": [1, 4], "size": 100}, // Overlap between Polytheistic and Agnostic/Atheistic
        {"sets": [2, 4], "size": 100}, // Overlap between Pantheistic and Agnostic/Atheistic
        // No overlaps for disjoint sets
    ];
    
    // Example of a disjoint set (not included in overlaps)
    var disjointSet = {"label": "Disjoint Set", "size": 1000}; // This is separate and not included in overlaps
  ...
  // Implement force simulation for labels
  var simulation = d3.forceSimulation(diagram.text)
      .force("charge", d3.forceManyBody().strength(-30))
      .force("center", d3.forceCenter(500 / 2, 500 / 2))
      .on("tick", function() {
          diagram.text.attr("x", function(d) { return d.x; })
                  .attr("y", function(d) { return d.y - (d.radius + 10); });
          diagram.svg.selectAll(".line")
              .attr("x1", function(d) { return d.x; })
              .attr("y1", function(d) { return d.y; })
              .attr("x2", function(d) { return d.x; })
              .attr("y2", function(d) { return d.y - (d.radius + 10); });
