        ...
        function chart(selection) {
            ...
            svg.selectAll("text").each(function(d) {
                ...
                labelNodes.push({
                    x: +textNode.attr("x"),
                    y: +textNode.attr("y"),
                    radius: 10, // approximate radius for label
                    textNode: textNode,
                    data: d
                });
            });

            var simulation = d3.forceSimulation(labelNodes)
                .force("charge", d3.forceManyBody().strength(-30))
                .force("center", d3.forceCenter(width / 2, height / 2))
                .force("collide", d3.forceCollide(function(d) { return d.radius; }))
                .on("tick", function() {
                    labelNodes.forEach(function(d) {
                        d.textNode.attr("x", d.x).attr("y", d.y);
                    });
                    updateLeadlinesAndLabels(); // Call to update leadlines on tick
                });

            // Restart the simulation
            simulation.alpha(1).restart();

            // Assuming 'circles' is an array of circle objects with x, y, and radius properties
            ...
            svg.selectAll(".line")
                ...
                .attr("x2", function(d) { return d.x; }) // Will update in simulation
                .attr("y2", function(d) { return d.y - (d.radius + 10); }) // Will update in simulation
                .attr("stroke", "black");
